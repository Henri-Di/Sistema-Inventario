...
$fornecedor = $_POST['Fornecedor'] ?? ''; // Obtém o valor do campo 'Fornecedor' do formulário ou uma string vazia se não estiver definido

// Converter o nome do fornecedor para maiúsculo
$fornecedor_upper = strtoupper($fornecedor); // Converte o nome do fornecedor para maiúsculas
...
$stmt_check = $conn->prepare("SELECT FORNECEDOR FROM FORNECEDOR WHERE FORNECEDOR = ?"); // Prepara a consulta SQL para verificar se o fornecedor já existe
$stmt_check->bind_param("s", $fornecedor_upper); // Associa o parâmetro ao prepared statement
...
if ($stmt_check->num_rows > 0) {
    ...
    $stmt_insert = $conn->prepare("INSERT INTO FORNECEDOR (FORNECEDOR) VALUES (?)"); // Prepara a consulta SQL para inserir um novo fornecedor
    $stmt_insert->bind_param("s", $fornecedor_upper); // Associa o parâmetro ao prepared statement
