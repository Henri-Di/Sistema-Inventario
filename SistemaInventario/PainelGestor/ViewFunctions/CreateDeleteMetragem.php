...
if (!isset($_SESSION['usuarioId']) || !isset($_SESSION['usuarioNome']) || !isset($_SESSION['usuarioCodigoP'])) {
    // Se qualquer uma das variáveis de sessão estiver ausente, redirecionar para a página de falha de autenticação
...
require_once('../../ViewConnection/ConnectionInventario.php'); // Inclui o arquivo de conexão ao banco de dados

// Obter o ID do parâmetro GET e sanitizar para garantir que é um número inteiro
$id = filter_input(INPUT_GET, 'id', FILTER_SANITIZE_NUMBER_INT); // Filtra e sanitiza o parâmetro 'id' da URL

// Verificar se o ID não está vazio
if (!empty($id)) {
    // Verificar a conexão com o banco de dados
    if ($conn->connect_error) {
        // Se houver erro na conexão, exibe uma mensagem e encerra o script
    ...
    try {
        ...
        $sql = "DELETE FROM METRAGEM WHERE id = ?"; // Consulta SQL para deletar um registro da tabela METRAGEM
        $stmt = $conn->prepare($sql); // Prepara a consulta SQL
        $stmt->bind_param("i", $id); // Faz o binding do parâmetro 'id' como inteiro

        // Executar a consulta SQL e verificar o resultado
        if ($stmt->execute() && $stmt->affected_rows > 0) {
            // Se a execução for bem-sucedida e o número de linhas afetadas for maior que zero, faz o commit da transação
            $conn->commit();
            // Redirecionar para a página de sucesso com uma mensagem
            header("Location: ../ViewSucess/SucessCreateDeleteMetragem.php?sucesso=" . urlencode("A metragem foi removida com sucesso"));
            exit(); // Interrompe a execução do script após o redirecionamento
        } else {
    ...
    // Se o ID estiver vazio, redirecionar para a página de falha com uma mensagem de erro
    header("Location: ../ViewFail/FailCreateDeleteMetragem.php?erro=" . urlencode("Não foi possível remover a metragem. Refaça a operação e tente novamente"));
    exit(); // Interrompe a execução do script após o redirecionamento
}
