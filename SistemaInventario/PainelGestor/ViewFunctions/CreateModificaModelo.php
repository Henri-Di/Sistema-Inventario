...
if (!empty($id) && isset($_POST['Modelo'])) {
    // Sanitizar o valor do modelo usando real_escape_string para prevenir injeção de SQL
    $modelo = $conn->real_escape_string($_POST['Modelo']); // Escapa caracteres especiais no valor do modelo para evitar SQL Injection

    // Construir a consulta SQL para atualização (usando prepared statement para segurança adicional)
    $sql = "UPDATE MODELO SET MODELO=? WHERE id=?"; // SQL para atualizar o valor do modelo baseado no ID
    $stmt = $conn->prepare($sql); // Prepara a consulta SQL
    $stmt->bind_param("si", $modelo, $id); // Associa os parâmetros à consulta SQL: o primeiro parâmetro é o valor do modelo e o segundo é o ID
    ...
    if ($stmt->execute()) {
        ...
        error_log("Erro na atualização do modelo: " . $stmt->error); // Registra o erro no log de erros
    ...
    // Se o ID estiver vazio ou o campo Modelo não for enviado, redirecionar para a página de falha
    header("Location: ../ViewFail/FailCreateModificaModelo.php?erro=" . urlencode("Não foi possível realizar a alteração no cadastro do modelo. Refaça a operação e tente novamente"));
    exit(); // Interrompe a execução do script após o redirecionamento
}
