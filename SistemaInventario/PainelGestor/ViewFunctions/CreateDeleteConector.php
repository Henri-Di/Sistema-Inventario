...
// 'Permissions-Policy' (anteriormente 'Feature-Policy') controla quais recursos podem ser acessados pelos navegadores e sites.
// - 'geolocation=(self)': Permite acesso à localização apenas para o mesmo domínio.
// - 'microphone=()': Nega o uso do microfone, evitando que o navegador peça permissão para acessar o microfone.
...
// Obter o ID do parâmetro GET e sanitizar para garantir que é um número inteiro
...
if (!empty($id)) {
    ...
    try {
        // Construir a consulta SQL para exclusão usando prepared statements para evitar SQL injection
        $sql = "DELETE FROM CONECTOR WHERE id = ?";
        ...
        if ($stmt->execute() && $stmt->affected_rows > 0) {
            ...
            $conn->commit();
            // Redirecionar para a página de sucesso se a remoção foi realizada
            header("Location: ../ViewSucess/SucessCreateDeleteConector.php?sucesso=" . urlencode("O conector foi removido com sucesso"));
            exit();
        } else {
            // Rollback da transação em caso de falha na exclusão
            $conn->rollback();
            // Redirecionar para a página de falha com mensagem apropriada
            header("Location: ../ViewFail/FailCreateDeleteConector.php?erro=" . urlencode("Não foi possível remover o conector. Refaça a operação e tente novamente"));
            exit();
        }
    } catch (Exception $e) {
        // Rollback da transação em caso de exceção para manter a consistência do banco de dados
        $conn->rollback();
    ...
    header("Location: ../ViewFail/FailCreateDeleteMetragem.php?erro=" . urlencode("Não foi possível remover a metragem. Refaça a operação e tente novamente"));
    exit();
}
