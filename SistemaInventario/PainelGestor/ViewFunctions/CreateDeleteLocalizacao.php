...
if (!empty($id)) {
    ...
    try {
        ...
        if ($stmt->execute() && $stmt->affected_rows > 0) {
            ...
            header("Location: ../ViewSucess/SucessCreateDeleteLocalizacao.php?sucesso=" . urlencode("A localização foi removida com sucesso"));
            exit();
        } else {
            // Rollback da transação em caso de falha na exclusão
            $conn->rollback();
            // Redirecionar para a página de falha com mensagem apropriada
            header("Location: ../ViewFail/FailCreateDeleteLocalizacao.php?erro=" . urlencode("Não foi possível remover a localização. Refaça a operação e tente novamente"));
            exit();
        }
    } catch (Exception $e) {
