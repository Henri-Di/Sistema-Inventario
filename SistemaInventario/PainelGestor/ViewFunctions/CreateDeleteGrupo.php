...
if (!empty($id)) {
    ...
    try {
        ...
        if ($stmt->execute() && $stmt->affected_rows > 0) {
            ...
            header("Location: ../ViewSucess/SucessCreateDeleteGrupo.php?sucesso=" . urlencode("O grupo foi removido com sucesso"));
            exit();
        } else {
            // Rollback da transação em caso de falha na exclusão
            $conn->rollback();
            // Redirecionar para a página de falha com mensagem apropriada
            header("Location: ../ViewFail/FailCreateDeleteGrupo.php?erro=" . urlencode("Não foi possível remover o grupo. Refaça a operação e tente novamente"));
            exit();
        }
    } catch (Exception $e) {
