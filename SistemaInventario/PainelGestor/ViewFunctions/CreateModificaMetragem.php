...
if (!empty($id) && isset($_POST['Metragem'])) {
    // Sanitizar o valor da metragem usando real_escape_string para prevenir injeção de SQL
    $metragem = $conn->real_escape_string($_POST['Metragem']); // Escapa caracteres especiais no valor da metragem para evitar SQL Injection

    // Construir a consulta SQL para atualização (usando prepared statement para segurança adicional)
    $sql = "UPDATE METRAGEM SET METRAGEM=? WHERE id=?"; // SQL para atualizar o valor da metragem baseado no ID
    $stmt = $conn->prepare($sql); // Prepara a consulta SQL
    $stmt->bind_param("si", $metragem, $id); // Associa os parâmetros à consulta SQL: o primeiro parâmetro é o valor da metragem e o segundo é o ID
    ...
    if ($stmt->execute()) {
        ...
        error_log("Erro na atualização da metragem: " . $stmt->error); // Registra o erro no log de erros
    ...
    // Se o ID estiver vazio ou o campo Metragem não for enviado, redirecionar para a página de falha
    header("Location: ../ViewFail/FailCreateModificaMetragem.php?erro=" . urlencode("Não foi possível realizar a alteração no cadastro da metragem. Refaça a operação e tente novamente"));
    exit(); // Interrompe a execução do script após o redirecionamento
}
