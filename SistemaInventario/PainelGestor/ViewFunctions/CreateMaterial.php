...
if (!isset($_SESSION['usuarioId']) || !isset($_SESSION['usuarioNome']) || !isset($_SESSION['usuarioCodigoP'])) {
    // Se as informações do usuário não estiverem disponíveis, redirecionar para a página de falha de autenticação
    ...
    exit(); // Interrompe a execução do script após o redirecionamento para evitar que o código subsequente seja executado
}
...
$material = $_POST['Material'] ?? ''; // Obtém o valor enviado pelo formulário ou define uma string vazia se o campo não estiver presente
...
// Converter o valor do material para letras maiúsculas, garantindo a correta manipulação de caracteres acentuados
$material = mb_strtoupper($material, 'UTF-8'); // Converte o valor para maiúsculas para manter a consistência no banco de dados

// Preparar e executar uma consulta SQL para verificar se o material já existe no banco de dados
$sql_check = "SELECT MATERIAL FROM MATERIAL WHERE MATERIAL = ?"; // Consulta SQL para verificar a existência do material
$stmt_check = $conn->prepare($sql_check); // Prepara a consulta SQL
$stmt_check->bind_param("s", $material); // Associa o parâmetro da consulta com o valor do material
$stmt_check->execute(); // Executa a consulta
$stmt_check->store_result(); // Armazena o resultado da consulta para verificar se o material já está presente

// Verificar se o material já existe
if ($stmt_check->num_rows > 0) {
    // Se o material já estiver registrado, redirecionar para a página de erro indicando que o material já existe
    ...
    // Se o material não existir, preparar a consulta SQL para inserir o novo material
    $sql_insert = "INSERT INTO MATERIAL (MATERIAL) VALUES (?)"; // Consulta SQL para inserir o novo material
    $stmt_insert = $conn->prepare($sql_insert); // Prepara a consulta SQL
    $stmt_insert->bind_param("s", $material); // Associa o parâmetro da consulta com o valor do material

    // Executar a consulta SQL para inserir o novo material
