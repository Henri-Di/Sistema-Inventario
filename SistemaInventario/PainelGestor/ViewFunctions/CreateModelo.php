...
$modelo = $_POST['Modelo'] ?? ''; // Obtém o valor do campo 'Modelo' enviado pelo formulário ou define uma string vazia se o campo não estiver presente

// Verificar a conexão com o banco de dados
if ($conn->connect_error) {
    // Se houver um erro na conexão, exibir a mensagem de erro e interromper a execução do script
    die("Falha na conexão: " . $conn->connect_error); // Termina a execução se a conexão falhar
}

// Converter o valor do modelo para letras maiúsculas, lidando com caracteres acentuados
$modelo = mb_strtoupper($modelo, 'UTF-8'); // Converte o valor do modelo para maiúsculas para garantir consistência no banco de dados

// Sanitizar os dados de entrada para evitar injeção de SQL usando prepared statement
$stmt_check = $conn->prepare("SELECT MODELO FROM MODELO WHERE MODELO = ?"); // Prepara uma consulta SQL para verificar se o modelo já existe
$stmt_check->bind_param("s", $modelo); // Associa o parâmetro da consulta com o valor do modelo
...
if ($stmt_check->num_rows > 0) {
    // Se o modelo já estiver cadastrado, redirecionar para a página de erro indicando duplicidade
    ...
    // Se o modelo não existir, preparar a consulta SQL para inserção
    $stmt_insert = $conn->prepare("INSERT INTO MODELO (MODELO) VALUES (?)"); // Prepara uma consulta SQL para inserir o novo modelo
    $stmt_insert->bind_param("s", $modelo); // Associa o parâmetro da consulta com o valor do modelo
