...
if (!empty($id) && isset($_POST['Localizacao'])) {
    // Sanitizar o valor da localização usando real_escape_string para prevenir injeção de SQL
    ...
    $sql = "UPDATE LOCALIZACAO SET LOCALIZACAO = ? WHERE id = ?"; // SQL para atualizar o valor da localização baseado no ID
    ...
    if ($stmt) {
        ...
        if ($stmt->affected_rows > 0) {
            ...
            // Se nenhuma linha foi afetada, isso pode significar que o ID não corresponde a nenhum registro existente ou que o valor não mudou
    ...
    // Se o ID estiver vazio ou o campo Localizacao não for enviado, redirecionar para a página de erro
    header("Location: ../ViewFail/FailCreateModificaLocalizacao.php?erro=" . urlencode("Não foi possível realizar a alteração no cadastro da localização. Refaça a operação e tente novamente"));
    exit(); // Interrompe a execução do script após o redirecionamento
}
