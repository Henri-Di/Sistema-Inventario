...
$id = filter_input(INPUT_GET, 'id', FILTER_SANITIZE_NUMBER_INT); // Obtém e sanitiza o ID passado no parâmetro GET para evitar injeção de código

// Verificar se os dados do formulário foram submetidos
if (!empty($_POST['id'])) {
    // Sanitizar os dados de entrada do formulário
    $id = $conn->real_escape_string($_POST['id']); // Sanitiza o ID da nota fiscal
    $numNotaFiscal = $conn->real_escape_string($_POST['NumNotaFiscal'] ?? ''); // Sanitiza o número da nota fiscal (pode ser uma string vazia se não estiver definido)
    $valorNotaFiscal = $conn->real_escape_string($_POST['ValorNotaFiscal'] ?? ''); // Sanitiza o valor da nota fiscal
    $material = $conn->real_escape_string($_POST['Material'] ?? ''); // Sanitiza o material
    $conector = $conn->real_escape_string($_POST['Conector'] ?? ''); // Sanitiza o conector
    $metragem = $conn->real_escape_string($_POST['Metragem'] ?? ''); // Sanitiza a metragem
    $modelo = $conn->real_escape_string($_POST['Modelo'] ?? ''); // Sanitiza o modelo
    $quantidade = $conn->real_escape_string($_POST['Quantidade'] ?? ''); // Sanitiza a quantidade
    $fornecedor = $conn->real_escape_string($_POST['Fornecedor'] ?? ''); // Sanitiza o fornecedor
    $dataRecebimento = $conn->real_escape_string($_POST['DataRecebimento'] ?? ''); // Sanitiza a data de recebimento
    $dataCadastro = $conn->real_escape_string($_POST['DataCadastro'] ?? ''); // Sanitiza a data de cadastro
    $datacenter = $conn->real_escape_string($_POST['DataCenter'] ?? ''); // Sanitiza o nome do datacenter

    // Verificar se o datacenter existe
    $sqlDatacenter = "SELECT IDDATACENTER FROM DATACENTER WHERE NOME = ?"; // Consulta para obter o ID do datacenter com base no nome
    $stmtDatacenter = $conn->prepare($sqlDatacenter); // Prepara a consulta SQL
    $stmtDatacenter->bind_param("s", $datacenter); // Associa o parâmetro do nome do datacenter
    ...
    if ($stmtDatacenter->num_rows === 0) {
        // Se nenhum datacenter foi encontrado, redirecionar para a página de falha
    ...
    $stmtDatacenter->close(); // Fecha o statement do datacenter

    // Construir a consulta SQL para atualização da nota fiscal (usando prepared statement para segurança adicional)
    ...
    $stmt->bind_param("sssssssssssi", $numNotaFiscal, $valorNotaFiscal, $material, $conector, $metragem, $modelo, $quantidade, $fornecedor, $dataRecebimento, $dataCadastro, $idDatacenter, $id); // Associa os parâmetros à consulta SQL
    ...
    if ($stmt->execute()) {
        ...
        // Se a execução da consulta falhar, redirecionar para a página de falha
    ...
    // Se o ID não estiver presente ou se não houver dados submetidos, redirecionar para a página de falha
    header("Location: ../ViewFail/FailCreateModificaNotaFiscal.php?erro=" . urlencode("Não foi possível realizar a alteração da nota fiscal. Refaça a operação e tente novamente"));
    exit(); // Interrompe a execução do script após o redirecionamento
}
