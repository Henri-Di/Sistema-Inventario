...
if (!isset($_SESSION['usuarioId']) || !isset($_SESSION['usuarioNome']) || !isset($_SESSION['usuarioCodigoP'])) {
    // Se o usuário não estiver autenticado, redirecionar para a página de erro
...
$localizacao = $_POST['Localizacao'] ?? ''; // Usa o operador de coalescência nula para definir um valor padrão se a variável não estiver definida

// Verificar se a conexão com o banco de dados foi estabelecida com sucesso
if ($conn->connect_error) {
    // Se houver um erro na conexão, exibir a mensagem de erro e parar a execução
...
$localizacao = mb_strtoupper($localizacao, 'UTF-8'); // Converte a localização para maiúsculas, garantindo o suporte a caracteres especiais

// Sanitizar os dados de entrada para prevenir injeções de SQL usando prepared statement
// Preparar a consulta SQL para verificar se a localização já existe
$sql_check = "SELECT LOCALIZACAO FROM LOCALIZACAO WHERE LOCALIZACAO = ?";
$stmt_check = $conn->prepare($sql_check); // Prepara a consulta SQL para execução
$stmt_check->bind_param("s", $localizacao); // Associa o parâmetro da consulta
$stmt_check->execute(); // Executa a consulta
$stmt_check->store_result(); // Armazena o resultado da consulta

// Verificar se a localização já existe no banco de dados
if ($stmt_check->num_rows > 0) {
    // Se a localização já estiver cadastrada, redirecionar para a página de falha
    ...
    // Se a localização não existir, preparar a consulta SQL para inserção
    $sql_insert = "INSERT INTO LOCALIZACAO (LOCALIZACAO) VALUES (?)"; // Consulta SQL para inserção de uma nova localização
    $stmt_insert = $conn->prepare($sql_insert); // Prepara a consulta SQL para execução
    $stmt_insert->bind_param("s", $localizacao); // Associa o parâmetro da consulta

    // Executar a consulta SQL para inserir a nova localização
    if ($stmt_insert->execute()) {
        ...
        // Se a inserção falhar, redirecionar para a página de falha
